      ...
      return (
        <div key={channel.id} className="flex items-center gap-1 px-1 py-0.5">
          <input
            ...
            autoFocus
            className="flex-1 px-1.5 py-0.5 text-sm bg-gray-600 border border-blue-500 rounded text-white outline-none"
          />
    ...
    return (
      <div
        ...
        className={`group flex items-center rounded transition-colors ${
          currentChannelId === channel.id ? 'bg-gray-700' : 'hover:bg-gray-700/50'
        }`}
      >
        <button
          onClick={() => onSelectChannel(channel.id)}
          className={`flex-1 text-left px-2 py-1.5 flex items-center gap-2 ${
            currentChannelId === channel.id ? 'text-white' : 'text-gray-400 group-hover:text-gray-200'
          }`}
        >
          {icon}
        ...
        {isOwner && (
          <div className="flex items-center gap-0.5 pr-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              onClick={e => startRename(channel, e)}
              className="p-0.5 text-gray-400 hover:text-blue-400 transition-colors"
    ...
    <div className="space-y-4">
      {textChannels.length > 0 && (
        <div>
          ...
          {textChannels.map(channel => renderChannel(channel,
      ...
      {voiceChannels.length > 0 && (
        <div>
          ...
          {voiceChannels.map(channel => renderChannel(channel,
