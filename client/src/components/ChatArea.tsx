...
export default function ChatArea({ channel, messages, onSendMessage }: ChatAreaProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Channel header */}
      <div className="h-12 border-b border-gray-700 px-4 flex items-center justify-between bg-gray-800">
          ...
          <form onSubmit={handleSubmit}>
            <div className="flex gap-2 items-center">
              ...
              <div className="relative group">
                <button type="button" className="w-10 h-10 flex items-center justify-center bg-gray-700 hover:bg-gray-600 rounded-full transition-colors text-gray-300 hover:text-white">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                  </svg>
                </button>
                {/* Dropdown menu - hidden for now */}
                <div className="hidden group-hover:block absolute bottom-full left-0 mb-2 w-48 bg-gray-800 rounded-lg shadow-xl border border-gray-700 py-1">
              ...
              <div className="flex-1 relative">
                <input
                  ...
                  className="w-full pl-4 pr-20 py-3 bg-gray-700 border border-gray-600 rounded-md text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-transparent"
                  maxLength={2000}
                />
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                  {/* GIF button */}
                  <button type="button" className="px-2 py-1 text-xs font-bold text-gray-400 hover:text-white transition-colors rounded">
                    GIF
                  </button>
                  {/* Send button - only visible when there's text */}
                  {input.trim() && (
                    <button type="submit" className="p-1.5 bg-gray-600 hover:bg-gray-500 rounded text-white transition-colors">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
