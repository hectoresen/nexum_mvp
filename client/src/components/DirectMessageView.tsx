    ...
    <div className="flex flex-col h-full">
      ...
      <div className={`h-12 px-4 border-b ${tw.borderDefault} flex items-center gap-3 flex-shrink-0 ${tw.bgHeader}`}>
        <div className={`w-8 h-8 rounded-full ${tw.bgInput} flex items-center justify-center flex-shrink-0 overflow-hidden`}>
          {otherAvatarUrl ? (
            <img src={otherAvatarUrl} alt={otherUser.username} crossOrigin={otherAvatarUrl.startsWith('http://') ? 'anonymous' : undefined} className="w-full h-full object-cover" />
          ) : (
        ...
        {messages.length === 0 && (
          <div className={`flex flex-col items-center justify-center h-full gap-3 ${tw.textMuted}`}>
            <div className={`w-16 h-16 rounded-full ${tw.bgInput} flex items-center justify-center`}>
              {otherAvatarUrl ? (
                <img src={otherAvatarUrl} alt={otherUser.username} crossOrigin={otherAvatarUrl.startsWith('http://') ? 'anonymous' : undefined} className="w-full h-full object-cover rounded-full" />
              ) : (
                  ...
                  <div className="w-10 flex-shrink-0">
                    {showAvatar && (
                      <div className={`w-10 h-10 rounded-full ${tw.bgInput} flex items-center justify-center overflow-hidden`}>
                        {senderAvatar ? (
                          <img src={senderAvatar} alt={msg.sender_username} crossOrigin={senderAvatar.startsWith('http://') ? 'anonymous' : undefined} className="w-full h-full object-cover" />
                        ) : (
