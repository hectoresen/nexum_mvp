  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg p-6 w-[480px] max-h-[70vh] flex flex-col shadow-xl">
        ...
        {users && users.length > 0 && (
          <>
            <p className="text-sm text-gray-400 mb-3">{users.length} user{users.length !== 1 ? 's' : ''} registered</p>
            <div className="flex-1 overflow-y-auto space-y-1">
              {users.map(user => (
                <div
                  key={user.id}
                  className="flex items-center gap-3 px-3 py-2 bg-gray-700/50 rounded-md"
                >
